...
func (cr *crust) shift(g *Grid, plates []Plate, step *[plateCap][2]int) {
	...
	for i := range cr.plate {
		...
		cr.nplate[j], cr.norg[j], cr.nfresh[j], cr.nborn[j], cr.nocean[j] = cr.plate[i], cr.org[i], cr.fresh[i], cr.born[i], cr.ocean[i]
		cr.nbuilt[j] = cr.built[i]
	}
	cr.openFloor(g)
...
func (w *Land) tectonics(g *Grid, plates []Plate, cr *crust, book []record, epoch int, gap float64, touch, weld, grain, bow []float64) {
	...
	for i := range g.Tiles {
		t := &g.Tiles[i]
		mine := plates[t.Plate]
		worst, j, ok := g.meetingAt(plates, i)
		if !ok {
			continue
		}
...
func (w *Land) reshape(g *Grid, plates []Plate, fl *flooding, touch, weld []float64) []Plate {
	...
	for k := 0; k < stride; k++ {
		...
		to := uint8(len(plates))
		plates = append(plates, Plate{Ocean: plates[r].Ocean, into: to, grow: plates[r].grow})
		w.lean(&plates[to])
...
func (g *Grid) meetingAt(plates []Plate, i int) (worst float64, at int, ok bool) {
