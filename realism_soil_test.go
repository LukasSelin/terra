...
// today - how deep its soil is, its sand and clay, and the organic matter the
// fertility is read with - on the worlds the other yardsticks have already
// made. What a tile does not carry yet - a carbon stock, a soil order - stays
...
var soilYardsticks = []realYardstick{
	...
	// 11. Organic carbon by biome. What a tile has is how much organic matter
	// its soil holds against the map's middling climate - see organic - and
	// not a stock, so only its ratios between biomes can be read against the
	// earth's.
	{yardstick: yardstick{
		name: "soil organic matter, forest over desert (PET/P > 5), globe", unit: "x", scale: "water", lo: 1.5, hi: 3.2, slow: true,
		...
		measure: func() float64 { return organicByBiome(globes()).forestOverDesert },
	},
		gap: "known gap: I - organic is growth over decay held in 0.5-1.5, with no litter or roots of its own for a wood: the desert sits on the floor of 0.5 and forest reads 0.73, 1.45x",
	},
	{yardstick: yardstick{
		name: "soil organic matter, tundra (-15 to -3 C) over desert (PET/P > 5), globe", unit: "x", scale: "water", lo: 1.5, hi: 3.2, slow: true,
		...
		measure: func() float64 { return organicByBiome(globes()).tundraOverDesert },
	},
		gap: "known gap: I - nothing grows into organic below -5 C and no waterlogging or permafrost keeps it, so the tundra reads the desert's floor of 0.5: 1.0x",
	},
	{yardstick: yardstick{
		name: "mean soil organic carbon, top metre, land", unit: "kg C/m2", scale: "water", lo: 9, hi: 13, slow: true,
		...
		gap: "known gap: I - organic matter is a multiplier of 0.5-1.5 on what a soil holds, not a stock of carbon in kilograms a square metre",
	},

	// 12. The soil orders. The earth's ice-free land by the order of soil on
	// it. Nothing classifies a tile's soil yet: that needs the soil's state -
	// its age, leaching, carbonate, B horizon, permafrost - which a tile
	// does not carry (see S2-S4 of the tectonics and soil scope).
	{yardstick: yardstick{
		...
		gap: "known gap: I - no soil orders: a tile's soil is a depth and a sand/clay mixture, with no carbonate or salt build-up to call aridic",
	},
	{yardstick: yardstick{
		...
		gap: "known gap: I - no soil orders: permafrost is in the climate but no soil is churned or classified by it",
	},
	{yardstick: yardstick{
		...
		gap: "known gap: I - no soil orders: no exposure age or iron and aluminium enrichment to tell an old leached tropical soil",
	},
	{yardstick: yardstick{
		...
		gap: "known gap: I - no soil orders: grass and forest make the same soil, so there is no dark base-rich grassland epipedon",
	},
}
...
type organicReading struct{ forestOverDesert, tundraOverDesert float64 }

// organicByBiome is the mean of organic over forest, over tundra - open dry
// land whose year's mean is -15 to -3 C, the range the tundra's is - and over
// desert, open dry land whose aridity (PET over rain, as forestByAridity reads
// it) passes five, as ratios to the desert's.
func organicByBiome(gs []*Grid) organicReading {
	return remember(fmt.Sprintf("organicbiome/%p/%d", gs[0], len(gs)), func() organicReading {
		var fs, fn, ts, tn, ds, dn float64
		for _, g := range gs {
			if g.air == nil {
				continue
			}
		...
		r := organicReading{math.NaN(), math.NaN()}
		if dn == 0 {
			return r
		}
		d := ds / dn
