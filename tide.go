...
// gathers the tide - see Grid.TidalRange - multiplies all of it.
type Tide struct {
	Tick      int
	Moon      Moon
	High, Low float64
...
func epochOf(seed uint64) Epoch {
	return Epoch{
		Synodic:     unit(splitmix(seed^0x6d6f6f6e)) * clock.SynodicMonth,
		Anomalistic: unit(splitmix(seed^0x70657269)) * clock.AnomalisticMonth,
	}
}
...
func MoonOn(tick int, e Epoch) Moon {
	age := mod(float64(tick)+e.Synodic, clock.SynodicMonth)
	return Moon{Age: age, Phase: age / clock.SynodicMonth}
}
...
func TideOn(tick int, e Epoch) Tide {
	m := MoonOn(tick, e)
	apart := 2 * math.Pi * (m.Age - SpringLag) / clock.SynodicMonth
	near := math.Cos(2 * math.Pi * mod(float64(tick)+e.Anomalistic, clock.AnomalisticMonth) / clock.AnomalisticMonth)
