...
// says so here. The tile is one an arc raised, so that every link of a height's
...
// meeting had raised; tile 119 was raised by an arc twelve million years
// before, by some 5.4 km of the history's own.
func TestTheChainForOneTileOfTheAncientValley(t *testing.T) {
	...
	if chain[0].Note != "arc" || chain[0].When != 3*epochYears || chain[0].Quantity <= 0 {
		t.Errorf("tile 119 was raised by %q %v years ago by %v m; want an arc, %v years ago, more than nothing",
			chain[0].Note, chain[0].When, chain[0].Quantity, 3*epochYears)
	}
